...
class Urls(View, Database, Manager):
    ...
    def page_11(self):
        ...
        if validator_1 is True:
            ...
            select = Manager.parse_select_of_players(self, select)
            validator_2, errors_2 = Manager.check_selection_of_players(self, select)
            if validator_2:
                instances = Database.select_from_player_table(self, get_instance=True, where_id=select)
                validator_3, errors_3 = Manager.check_instances_players(self, select, instances)
                if validator_3:
